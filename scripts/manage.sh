...
install(){    
    if [ -d "/root/minerproxy_pro" ]; then
        echo -e "檢測到您已安裝MinerProxy-Pro，請勿重複安裝，如您確認您未安裝請使用rm -rf /root/minerproxy_pro指令" && exit 1
    fi
    if screen -list | grep -q "minerproxy_pro"; then
        echo -e "檢測到您的MinerProxy-Pro已啟動，請勿重複安裝" && exit 1
    fi
    ...
    wget https://raw.githubusercontent.com/Minefrok/MinerProxy-Pro/main/scripts/run.sh -O /root/minerproxy_pro/run.sh
    ...
    screen -dmS minerproxy_pro
    sleep 0.2s
    screen -r minerproxy_pro -p 0 -X stuff "cd /root/minerproxy_pro"
...
uninstall(){
    read -p "您確認您是否刪除MinerProxy-Pro)[yes/no]：" flag
    if [ -z $flag ];then
         echo "您未正確輸入" && exit 1
    else
        if [ "$flag" = "yes" -o "$flag" = "ye" -o "$flag" = "y" ];then
            screen -X -S minerproxy_pro quit
            rm -rf /root/minerproxy_pro
            echo "MinerProxy-Pro已成功從您的伺服器上卸載"
        fi
    fi
}
...
update(){
    ...
    rm -rf /root/minerproxy_pro/MinerProxy-Pro
    ...
    sleep 2s
    echo "MinerProxy-Pro 已經更新至V3.0.0版本並啟動"
    cat /root/minerproxy_pro/pwd.txt
    echo ""
...
start(){
    if screen -list | grep -q "minerproxy_pro"; then
        echo -e "檢測到您的MinerProxy-Pro已啟動，請勿重複啟動" && exit 1
    fi
...
echo "======================================================="
echo "MinerProxy-Pro 一鍵腳本，脚本默认安装到/root/minerproxy_pro"
echo "                                   腳本版本：V3.0.0"
echo "  1、安  装"
echo "  2、卸  载"
echo "  3、更  新"
